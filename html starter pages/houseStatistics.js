...
fetch("https://api.propublica.org/congress/v1/113/house/members.json", {
    ...
    console.log(data);
    countMembers(houseMembers, summary);
    makeRow(summary);
    houseMembers.sort(sortByMissedAscending);
    var myTopTen = topTenPercent(houseMembers);
    makeRowAttendance(myTopTen);
    houseMembers.sort(sortByMissedAscending2);
    var myTopTen = topTenPercent(houseMembers);
...
function countMembers(houseMembers, summary) {
    ...
    for (var i = 0; i < houseMembers.length; i++) {
        ...
        if (houseMembers[i].party === "R" && houseMembers[i].votes_with_party_pct != undefined) {

            republicans += 1;
            republicanVotes += houseMembers[i].votes_with_party_pct;
        } else if (houseMembers[i].party === "D" && houseMembers[i].votes_with_party_pct != undefined) {
            democrats += 1;
            democraticVotes += houseMembers[i].votes_with_party_pct;
        } else if (houseMembers[i].party === "I" && houseMembers[i].votes_with_party_pct != undefined) {
            independents += 1;
            independentVotes += houseMembers[i].votes_with_party_pct;
        }
...
function makeRow(summary) {
    ...
    var tbody = document.getElementById("houseTotals");
...
function makeRowAttendance(membersArray) {
    var tbody = document.getElementById("house-attendance");
    ...
    for (var i = 0; i < membersArray.length; i++) {
        ...
        var td = document.createElement("td");
        td.innerHTML = membersArray[i].missed_votes;
        tr.appendChild(td);


        var td = document.createElement("td");
        td.innerHTML = membersArray[i].missed_votes_pct;
        tr.appendChild(td);
...
function topTenPercent(members) {
    ...
    if (leastEngaged[44].missed_votes_pct === members[i].missed_votes_pct){
        leastEngaged.push(members[i]);
    }
        }
    return leastEngaged;
...
function makeRowAttendance2(membersArray) {
    var tbody = document.getElementById("house-attendance2");
